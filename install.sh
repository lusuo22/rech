...
# --- 0. 环境初始化 ---
set -e  # 遇到错误立即退出[cite: 3]

# 识别真实的执行用户（防止 sudo 模式下路径全变成 /root）
...
# 强制切换到工作目录，确保文件下载位置固定[cite: 3]
WORKDIR="$REAL_HOME/rech"
mkdir -p "$WORKDIR" && cd "$WORKDIR"

# --- 1. 基础工具准备 ---
echo ">>> [1/9] 更新系统并安装基础工具..."
apt update && apt install -y wget unzip curl[cite: 3]
...
echo "  提示：请重新打开终端或执行 'source ~/.bashrc' 激活环境"
echo "========================================"